...
import villageData from '../assets/Bot Data/villageData.json';
...
const Chatbot = () => {
    ...
    const callPrimaryAPI = async (message, contextPrompt) => {
        ...
        const response = await fetch(
            `${`https://api.fasturl.link/aillm/gpt-4o-mini`}?ask=${encodeURIComponent(
                message
            )}&style=${encodeURIComponent(contextPrompt)}&sessionId=${sessionId}`
        );
        ...
        return data.result || 'Maaf, tidak ada respons dari AI.';
    };

    const callFallbackAPI = async (message, contextPrompt) => {
        const url = `${import.meta.env.VITE_GEMINI_API_URL}?key=${import.meta.env.VITE_GEMINI_API_KEY}`;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            try {
                ...
                botResponse = await callPrimaryAPI(newMessage, contextPrompt);
            } catch (primaryError) {
                console.log('Primary API failed, trying fallback API...', primaryError);
                // If primary fails, try fallback API
                botResponse = await callFallbackAPI(newMessage, contextPrompt);
            }
            ...
            const errorMsg = error.message.includes('404')
                ...
                : error.message.includes('CORS')
                    ...
                    : 'Terjadi kesalahan{ERR NF01_ALL_API }.';
            setError(errorMsg);
            ...
            <div className="flex items-start gap-3">
                ...
                <div className="flex-grow min-w-0">
                    <div className="flex items-center justify-between gap-4 mb-2">
                        <h4 className="font-medium text-black">{isBot ? 'Tunjungtirto Bot' : 'You'}</h4>
                ...
                <div className="p-6 border-b border-white/10" data-aos="fade-down" data-aos-duration="800">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-2xl font-bold mb-3 text-transparent bg-clip-text bg-gradient-to-r from-[#6366f1] to-[#a855f7]">
                            Chat with Tunjungtirto Bot <span className="text-xl text-indigo-400">({chatMessages.length})</span>
                        </h3>
                    </div>
                </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="space-y-2">
                            ...
                            <textarea
                                ...
                                placeholder="Ask about Desa Tunjungtirto..."
                    ...
                    <div className="space-y-4 h-[300px] overflow-y-auto custom-scrollbar" id="chat-history">
                        {chatMessages.length === 0 ? (
                            <div className="text-center py-8">
                                ...
                                <p className="text-gray-400">No messages yet. Ask about Desa Tunjungtirto!</p>
                            </div>
                        ) : (
