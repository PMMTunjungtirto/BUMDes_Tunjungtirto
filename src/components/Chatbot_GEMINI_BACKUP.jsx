...
const Chatbot = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.log('Context size:', contextPrompt.length);
                            ...
                            contents: [
                                {
                                    parts: [
                                        {
                                            text: `${contextPrompt}\nPertanyaan pengguna: ${newMessage}`,
                                        },
                                    ],
                                },
                            ],
            ...
            if (!response.ok) {
                ...
                throw new Error(`API request failed with status ${response.status}: ${statusText}`);
            }

            const data = await response.json();
            const botResponse = data.candidates?.[0]?.content?.parts?.[0]?.text || 'Maaf, tidak ada respons dari API.';
            ...
            let errorMsg = 'Terjadi kesalahan saat menghubungi server. Silakan coba lagi nanti.';
            if (error.message.includes('404')) {
                errorMsg = 'API endpoint tidak ditemukan. Periksa konfigurasi API.';
            } else if (error.message.includes('CORS')) {
                errorMsg = 'CORS error: API tidak mengizinkan permintaan langsung dari browser.';
            } else if (error.message.includes('timed out')) {
                errorMsg = 'Permintaan ke server terlalu lama. Coba lagi nanti.';
            } else if (error.message.includes('401')) {
                errorMsg = 'API key tidak valid. Periksa kembali API key Anda.';
            } else if (error.message.includes('400')) {
                errorMsg = 'Permintaan tidak valid. Periksa format data yang dikirim.';
            }
