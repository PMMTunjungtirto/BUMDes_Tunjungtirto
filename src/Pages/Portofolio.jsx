...
import { ShoppingBasket, Award, Book, CloudLightning } from "lucide-react";
...
const techStacks = [
  ...
  { icon: "public/bootstrap.svg", language: "Bootstrap" },
  { icon: "public/firebase.svg", language: "Firebase" },
  { icon: "public/MUI.svg", language: "Material UI" },
  { icon: "public/vercel.svg", language: "Vercel" },
  { icon: "public/SweetAlert.svg", language: "SweetAlert2" },
];
...
export default function FullWidthTabs() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const pujaseraData = pujaseraSnapshot.docs.map((doc) => ({ // Data untuk pujasera
        id: doc.id,
        ...doc.data(),
        TechStack: doc.data().TechStack || [],
      }));
  ...
  return (
      <div className="md:px-[10%] px-[5%] w-full sm:mt-0 mt-[3rem] bg-[#FFFDF6] overflow-hidden" id="Portofolio">
        ...
        <Box sx={{ width: "100%" }}>
          {/* AppBar and Tabs section - updated with new tab */}
          <AppBar
              position="static"
              elevation={0}
              sx={{
          ...
          >
            ...
            >
              <Tab
                  icon={<ShoppingBasket className="mb-2 w-5 h-5 transition-all duration-300" />}
                  label="Klinik Tirto Husada"
                  {...a11yProps(0)}
              />
              <Tab
                  icon={<Book className="mb-2 w-5 h-5 transition-all duration-300" />}
                  label="Pujasera Kembang Banyu"
                  {...a11yProps(1)}
              />
              <Tab
                  icon={<Book className="mb-2 w-5 h-5 transition-all duration-300" />}
          ...
          >
            ...
            {/*TabPanel untuk pujasera*/}
            <TabPanel value={value} index={1} dir={theme.direction}>
              <div className="container mx-auto flex justify-center items-center overflow-hidden">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-3 gap-5">
                  {displayedPujasera.map((pujasera, index) => (
                      <div
                          key={pujasera.id || index}
                          data-aos={index % 3 === 0 ? "fade-up-right" : index % 3 === 1 ? "fade-up" : "fade-up-left"}
                          data-aos-duration={index % 3 === 0 ? "1000" : index % 3 === 1 ? "1200" : "1000"}
                      >
                        <CardPujasera
                            ...
                            Description={pujasera.Description}
                            Link={pujasera.Link}
                            id={pujasera.id}
                        />
                      </div>
              ...
              {pujasera.length > initialItems && (
                  <div className="mt-6 w-full flex justify-start">
                    <ToggleButton
                        onClick={() => toggleShowMore('pujasera')}
                        isShowingMore={showAllPujasera}
                    />
                  </div>
              )}
            </TabPanel>
